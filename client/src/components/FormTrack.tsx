...
import { $addTrack, $editTrack } from '../graphql/mutation';

type Props = {
  editableTrack?: ITrack;
  refetchAllTracks: () => void;
  setEditableTrack: React.Dispatch<React.SetStateAction<ITrack | undefined>>;
};

const chords = ['A', 'B', 'H', 'C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#'];

export const FormTrack: FC<Props> = ({ editableTrack, setEditableTrack, refetchAllTracks }) => {
  const [addTrackMutation] = useMutation($addTrack);
  const [editTrackMutation] = useMutation($editTrack);
      ...
      await editTrackMutation({
        variables: {
          ...
          track: {
            title: data.title,
            artist: data.artist,
            blocks: data.blocks.map((b) => ({ title: b.title, text: b.text })),
