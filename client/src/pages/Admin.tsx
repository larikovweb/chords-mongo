...
import { $removeTrack } from '../graphql/mutation';
...
const Admin: FC = () => {
  ...
  } = useQuery<{ tracks: ITrack[] }>($QueryTracks);
  const [removeTrackMutation] = useMutation($removeTrack);
        ...
        <DeleteBtn
          onClick={(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
            ...
            if (window.confirm('Вы уверены, что хотите удалить трек?')) {
              removeTrackMutation({ variables: { id: track.id } });
              refetchAllTracks();
            }
          }}>
          <IconDelete />
  ...
  return (
    <Wrapper>
      <FormTrack
        refetchAllTracks={refetchAllTracks}
