...
import { $QueryTrack } from '../graphql/query';
...
const Track: FC = () => {
  ...
  const {
    ...
    error,
    data,
  } = useQuery<{ track: ITrack }>($QueryTrack, {
    variables: {
      id: trackId,
    },
  });
  ...
  const content = data && <ViewResult track={data.track} />;
...
const ViewResult: FC<{ track: ITrack }> = ({ track }) => {
  ...
  const chordsMinor = [...chordsMinorView.slice(index), ...chordsMinorView.slice(0, index)];
